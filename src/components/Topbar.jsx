...
export default function Topbar({ userName, role, roles, onRoleChange, dateLabel, onOpenMenu }) {
  return (
    <header className="flex flex-wrap items-center justify-between gap-4 px-4 sm:px-6 lg:px-10 py-4 sm:py-5 border-b border-hairline bg-paper/80 backdrop-blur sticky top-0 z-10">
      <div className="flex items-center gap-3 min-w-0">
        <button
          type="button"
          onClick={onOpenMenu}
          aria-label="Open navigation menu"
          className="lg:hidden shrink-0 w-10 h-10 flex items-center justify-center rounded-sm border border-hairline text-ink hover:border-brass transition-colors"
        >
          <Menu size={19} />
        </button>
        <div className="min-w-0">
          <p className="font-display text-xl sm:text-2xl leading-tight truncate">
            {role === 'Chairman' || role === 'Vice Chairman' || role === 'Super Admin' ? `Office of the ${role}` : `${role} Desk`}
          </p>
          <p className="text-sm text-ink-muted font-body mt-0.5 truncate">
